...
ingresados, como la contraseña) e informar el éxito de la operación. 
IMPORTANCIA SEGÚN MÉTODO MoSCoW: MUST HAVE (DEBE TENER)
ESTIMACIÓN INICIAL CON T-SHIRT SIZING: S
¿REQUIERE INVESTIGACIÓN TÉCNICA? (SPIKE): NO
SUPUESTOS: el sistema tiene conectividad con la base de datos, se tiene conexión a internet durante todo el proceso, el servidor de correo está disponible.
...
permitir la modificación del nombre de usuario e informar el éxito de la operación.
IMPORTANCIA SEGÚN MÉTODO MoSCoW: MUST HAVE (DEBE TENER)
ESTIMACIÓN INICIAL CON T-SHIRT SIZING: XS
¿REQUIERE INVESTIGACIÓN TÉCNICA? (SPIKE): NO 
SUPUESTOS: el usuario está autenticado correctamente, el backend tiene acceso a los datos actuales del usuario.
...
ENTONCES el sistema deberá permitir la eliminación del mismo y emitir el mensaje "CUENTA DE USUARIO ELIMINADA CORRECTAMENTE".
IMPORTANCIA SEGÚN MÉTODO MoSCoW: MUST HAVE (DEBE TENER)
ESTIMACIÓN INICIAL CON T-SHIRT SIZING: XS
¿REQUIERE INVESTIGACIÓN TÉCNICA? (SPIKE): NO 
SUPUESTOS: el usuario que solicita la eliminación o dió pie a ella está correctamente autenticado, se realiza una confirmación explícita antes de eliminar.
...
CUANDO se registre el vehículo con patente "ZZ000AA", ENTONCES el sistema deberá añadir el vehículo a un catálogo e informar el éxito de la operación. 
IMPORTANCIA SEGÚN MÉTODO MoSCoW: MUST HAVE (DEBE TENER) 
ESTIMACIÓN INICIAL CON T-SHIRT SIZING: S
¿REQUIERE INVESTIGACIÓN TÉCNICA? (SPIKE): NO
SUPUESTOS: el usuario que registra el vehículo tiene permisos adecuados, el sistema está sincronizado con la base de datos central.
...
ENTONCES el sistema deberá permitir la modificación del precio del vehículo e informar el éxito de la operación.
IMPORTANCIA SEGÚN MÉTODO MoSCoW: MUST HAVE (DEBE TENER)
ESTIMACIÓN INICIAL CON T-SHIRT SIZING: XS
¿REQUIERE INVESTIGACIÓN TÉCNICA? (SPIKE): NO
...
ENTONCES el sistema deberá permitir la eliminación del mismo y emitir el mensaje "VEHÍCULO CON PATENTE ABC123CD ELIMINADO CORRECTAMENTE".
IMPORTANCIA SEGÚN MÉTODO MoSCoW: MUST HAVE (DEBE TENER) 
ESTIMACIÓN INICIAL CON T-SHIRT SIZING: XS
¿REQUIERE INVESTIGACIÓN TÉCNICA? (SPIKE): NO
...
el sistema realiza una confirmación antes de eliminar.
...
de marca "Toyota" y su respectiva información.
IMPORTANCIA SEGÚN MÉTODO MoSCoW: COULD HAVE (PODRÍA TENER)
ESTIMACIÓN INICIAL CON T-SHIRT SIZING: L
¿REQUIERE INVESTIGACIÓN TÉCNICA? (SPIKE): SI. En cuanto a la aplicación o implementación de algún algoritmo de búsqueda, por ejemplo.
...
lista o escribirla con asistencia (autocompletado).
...
de modelo "Mustang" relacionados a su marca correspondiente, así como también su respectiva información.
IMPORTANCIA SEGÚN MÉTODO MoSCoW: COULD HAVE (PODRÍA TENER)
ESTIMACIÓN INICIAL CON T-SHIRT SIZING: L
¿REQUIERE INVESTIGACIÓN TÉCNICA? (SPIKE): SI. En cuanto a la aplicación o implementación de algún algoritmo de búsqueda, por ejemplo.
...
lista o escribirlo con asistencia (autocompletado).
...
ENTONCES el sistema deberá permitir la reserva del mismo e informar el éxito de la operación.
IMPORTANCIA SEGÚN MÉTODO MoSCoW: MUST HAVE (DEBE TENER)
ESTIMACIÓN INICIAL CON T-SHIRT SIZING: M
¿REQUIERE INVESTIGACIÓN TÉCNICA? (SPIKE): NO 
SUPUESTOS: el vehículo a reservar está disponible, existe una política clara de tiempo de reserva, se genera un número de reserva.
...
sistema deberá permitir la cancelación de la misma y emitir el mensaje "RESERVA CANCELADA CORRECTAMENTE".
IMPORTANCIA SEGÚN MÉTODO MoSCoW: MUST HAVE (DEBE TENER)
ESTIMACIÓN INICIAL CON T-SHIRT SIZING: XS
¿REQUIERE INVESTIGACIÓN TÉCNICA? (SPIKE): NO
SUPUESTOS: la reserva existe y está activa, el usuario que cancela tiene permisos válidos.
...
ENTONCES el sistema deberá mostrar por pantalla el estado de ese vehículo, algo crucial ya que en base a eso se podrá crear la reserva o no.
IMPORTANCIA SEGÚN MÉTODO MoSCoW: MUST HAVE (DEBE TENER)
ESTIMACIÓN INICIAL CON T-SHIRT SIZING: XS
¿REQUIERE INVESTIGACIÓN TÉCNICA? (SPIKE): NO
SUPUESTOS: el estado del vehículo se obtiene en tiempo real, el mensaje es visible y comprensible para el usuario.
...
en el sistema ENTONCES se debe generar automáticamente una factura electrónica válida.
IMPORTANCIA SEGÚN MÉTODO MoSCoW: MUST HAVE (DEBE TENER)
ESTIMACIÓN INICIAL CON T-SHIRT SIZING: L
...
y el equipo no tiene experiencia previa con facturación electrónica o tributaria.
SUPUESTOS: todos los datos fiscales del cliente están completos y válidos, el sistema no permite anulaciones o notas de crédito.
RIESGOS POTENCIALES: duplicación de facturas, facturación sin venta confirmada.
##### FEATURE 13: GENERAR CONTRATO DE COMPRAVENTA
CRITERIO DE ACEPTACIÓN PARA FEATURE 13: DADO QUE la venta ya fue facturada, CUANDO el vendedor marca el contrato de compraventa como "finalizado" en el sistema
ENTONCES el sistema deberá crear un documento asociado al contrato de la compra del vehículo.
IMPORTANCIA SEGÚN MÉTODO MoSCoW: SHOULD HAVE (DEBERÍA TENER)
ESTIMACIÓN INICIAL CON T-SHIRT SIZING: M
...
cálculo de impuestos y estructura del documento.
SUPUESTOS: el sistema tiene acceso a los datos completos del vehículo y su transacción, el sistema puede generar el comprobante en el 
formato correcto (PDF, XML, etc).
...
ENTONCES el sistema mostrará por pantalla una recopilación de los datos y estadísticas más importantes relacionadas a las ventas.
IMPORTANCIA SEGÚN MÉTODO MoSCoW: SHOULD HAVE (DEBERÍA TENER)
ESTIMACIÓN INICIAL CON T-SHIRT SIZING: XL
¿REQUIERE INVESTIGACIÓN TÉCNICA? (SPIKE): NO
SUPUESTOS: el usuario tiene los permisos necesarios para generar el informe, el sistema tiene recursos suficientes para procesar y generar el informe.
...
ENTONCES el sistema mostrará por pantalla una recopilación de los datos y estadísticas más importantes relacionadas a los clientes.
IMPORTANCIA SEGÚN MÉTODO MoSCoW: SHOULD HAVE (DEBERÍA TENER)
ESTIMACIÓN INICIAL CON T-SHIRT SIZING: XL
¿REQUIERE INVESTIGACIÓN TÉCNICA? (SPIKE): NO
...
ENTONCES el sistema mostrará por pantalla una recopilación de los datos y estadísticas más importantes relacionadas a los vehículos.
IMPORTANCIA SEGÚN MÉTODO MoSCoW: SHOULD HAVE (DEBERÍA TENER)
ESTIMACIÓN INICIAL CON T-SHIRT SIZING: XL
¿REQUIERE INVESTIGACIÓN TÉCNICA? (SPIKE): NO
SUPUESTOS: el usuario tiene los permisos necesarios para generar el informe, el sistema puede manejar el volumen de datos de vehículos registrados.
