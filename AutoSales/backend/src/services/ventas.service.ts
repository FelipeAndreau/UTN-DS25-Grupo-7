...
/**
 * Completar una venta cambiando el estado del vehículo a 'Vendido'.
 */
export async function completarVenta(id: number): Promise<Venta> {
  try {
    // Obtener la venta para saber el vehiculoId
    ...
    // Retornar la venta (podría incluir el vehículo actualizado si se quiere)
    return venta;
  } catch (error) {
