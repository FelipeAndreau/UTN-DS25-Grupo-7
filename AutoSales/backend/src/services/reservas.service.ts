...
import { EstadoVehiculo } from "@prisma/client"; // Asegúrate de importar el tipo EstadoVehiculo desde Prisma
...
export const crearReserva = async (data: CrearReservaDTO): Promise<{ reserva: ReservaResponse; reservaClienteView: ReservaClienteView }> => {
  try {
    ...
    // Verificar si el cliente existe (excepto para cliente anónimo que se crea automáticamente)
    ...
    if (data.fechaVisita) {
      ...
      reservaData.fechaVisita = fechaVisita;
...
export const actualizarEstadoReserva = async (vehiculoId: string, estado: string): Promise<void> => {
  // Fix: Ensure vehiculoId is a number and estado matches the expected enum type
  const vehiculoIdNumber = parseInt(vehiculoId, 10); // Convert vehiculoId to number
  const estadoEnum: EstadoVehiculo = estado as EstadoVehiculo; // Cast estado to the expected enum type

  await prisma.vehiculo.update({
    where: { id: vehiculoIdNumber },
