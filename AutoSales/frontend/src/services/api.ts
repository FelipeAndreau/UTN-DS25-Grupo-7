...
export const ventasService = {
    ...
    completar: async (id: number): Promise<void> => {
        await fetchWithAuth(`${API_URL}/ventas/${id}/completar`, {
            method: 'PUT'
        });
    }
};
...
export const vehiculosService = {
    ...
    // Usar endpoint privado para admin que muestra todos los vehículos
    getAll: async (): Promise<Vehiculo[]> => {
        const response = await fetchWithAuth(`${API_URL}/vehiculos/admin`);
        const data = await response.json();
        return data.data || data; // Maneja tanto el formato nuevo como el antiguo
    },
