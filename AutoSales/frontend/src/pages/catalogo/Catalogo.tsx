...
const Catalogo = () => {
  ...
  const cargarDatos = async () => {
    ...
    setError(null);
    try {
      // Cargar vehículos disponibles (usando endpoint público)
      const vehiculosData = await vehiculosService.getAllPublic();
  ...
  const handleReservar = async (vehiculoId: number) => {
    ...
    if (!reservaForm.fechaVisita) {
      alert("Selecciona una fecha de visita");
      return;
    }
    ...
    try {
      ...
      alert("¡Reserva realizada con éxito! Fecha de visita: " + reservaForm.fechaVisita);
    } catch (err) {
  ...
  const vehiculosFiltrados = vehiculos
    .filter((vehiculo) => {
      ...
      // TEMPORALMENTE: Mostrar todos los vehículos (Disponible y Reservado)
      // TODO: En producción, solo mostrar vehiculo.estado === 'Disponible'
        ...
        <div className="p-6">
          ...
          {seccionActiva === "catalogo" && (
            <div>
              ...
              <div className="mb-4">
                <p className="text-gray-600">
                  Mostrando {vehiculosFiltrados.length} vehículo{vehiculosFiltrados.length !== 1 ? 's' : ''} disponible{vehiculosFiltrados.length !== 1 ? 's' : ''}
                </p>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {vehiculosFiltrados.map((vehiculo, idx) => (
                    <div key={`${vehiculo.id}-${idx}`} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
                      <div className="relative">
                        <img
                          src={vehiculo.imagen || "/images/fotosautos/default.png"}
                              ...
                              <div className="flex items-center">
                                <div className="flex-shrink-0 h-12 w-12">
                                  <img
                                    ...
                                    src={veh?.imagen || "/images/fotosautos/default.png"}
